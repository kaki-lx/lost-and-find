    ...
    <div class="container">
        ...
        <div class="process" v-if="isEdit">
            <el-steps :active="4" simple>
                ...
                <el-step title="等待认领" icon="el-icon-loading"></el-step>
                <el-step title="等待审核" icon="el-icon-s-order"></el-step>
                <el-step title="私下联系" icon="el-icon-phone-outline"></el-step>
            </el-steps>
        </div>
            ...
            <el-form :inline="true" :model="searchForm" class="demo-form-inline">
                <el-form-item label="物品类型">
                    <el-select v-model="searchForm.type" ref="typeSelect" placeholder="物品类型" style="width: 110px"
                        ...
                        <el-option v-for="item in type" :key="item.id" :label="item.label"
                            :value="item.value"></el-option>
                    </el-select>
                </el-form-item>

                <el-form-item label="关键词">
                    <el-input v-model.trim="searchForm.keyword" placeholder="物品名称/丢失地点" @change="keywordInput()"
                        @clear="keywordClear()" clearable></el-input>
                </el-form-item>

                <el-form-item>
                    <!-- <el-button type="primary" icon="el-icon-search" @click="searchBtn()">查询</el-button> -->
                    <el-button type="primary" @click="handleIssue()">我要发布招领信息</el-button>
                </el-form-item>
            </el-form>
        </div>
        ...
        <div class="edit-form">
            <el-form :model="form" :rules="rules" ref="ruleForm" v-if="isEdit" style="width:100%">
                <div><el-divider content-position="center" style="font-size: 20px">发布失物招领信息</el-divider></div>
                ...
                <el-row :gutter="20" style="margin-top: 40px">
                    ...
                    <el-col :span="10">
                        ...
                        <el-form-item label="地点" prop="location" :label-width="formLabelWidth">
                            <el-input v-model.trim="form.location" placeholder="请输入拾取地点" type="text" maxlength="10"
                                show-word-limit></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="10">
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="20">
                        <el-form-item label="图片" prop="article_pic" :label-width="formLabelWidth">
                            请上传物品图片，注意隐私！（至多1张）
                            <el-upload action="http://127.0.0.1:3007/my/uploadClaimPic" ref="upload"
        ...
        <div class="list" v-if="!isEdit">
            <!-- title 标题 -->
            <div><el-divider content-position="center">失物招领信息列表</el-divider></div>

            <!-- card 卡片 -->
            <el-row class="el-row" v-infinite-scroll="load" infinite-scroll-disabled="disabled">
                <el-card class="box-card" v-for="item in claimList.slice(0, count)" :key="item.id" shadow="hover">
                    <div class="resume-btn">
                        <div class="resume">{{ item.resume }}</div>
                        <div style="display: flex; gap: 5px;">
                            <el-button type="primary" size="mini" @click="handleClaim(item)"
                                v-show="isLogin">认领</el-button>
                            <el-button type="danger" size="mini" @click="handleReport(item, 1)"
                    ...
                    <div class="article_pic" style="text-align: center;">
                        ...
                        <el-image v-if="item.article_pic"
                            :preview-src-list="[require('../../../public/claim_pic/' + item.article_pic)]"
                            style="height: 120px;width: 120px;"
                            :src="require('../../../public/claim_pic/' + item.article_pic)"></el-image>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                date: [
                    { required: true, message: '请输入拾取时间', trigger: 'blur' },
                ],
            },
            type: [],
            searchForm: {
    ...
    methods: {
        ...
        getClaimList() {
            http({
                ...
                // console.log(res.data.data, "#获取claimList成功")
            }).catch((err) => {
        ...
        typeSelectClear() {
            if (this.searchForm.keyword) {
                this.claimList = this.$store.getters.getClaimList
                const claimList_resume = this.claimList.filter(item => item.resume.includes(this.searchForm.keyword))
                const claimList_location = this.claimList.filter(item => item.location.includes(this.searchForm.keyword))
                this.claimList = [...claimList_resume, ...claimList_location]
                this.claimList = this.claimList.filter(function (item, index, arr) { return arr.indexOf(item, 0) === index })  // 数据去重
            } else {
                this.claimList = this.$store.getters.getClaimList
            }
        },
        typeSelectChange() {
            if (this.searchForm.keyword) {
                ...
                this.claimList = this.claimList.filter(function (item, index, arr) { return arr.indexOf(item, 0) === index })  // 数据去重
                // 3.对类型进行筛选
                this.claimList = this.claimList.filter(item => item.type == this.searchForm.type)
            } else {
                this.claimList = this.$store.getters.getClaimList
                this.claimList = this.claimList.filter(item => item.type == this.searchForm.type)
            }
                ...
                if (valid) {
                    ...
                    if (this.files.length == 1) {
                        ...
                        http({
                            ...
                            // console.log(res.data.message,"#上传头像 更改 article_pic的 res")
                        }).catch((err) => {
                            console.log(err, "#上传头像 更改 article_pic的 err")
                        })
                    } else {
        ...
        handleClaim(item) {
            ...
            if (this.$store.getters.getUserInfo.claimLOA == '0') {
                this.$message.warning('您没有认领权限！')
                return
            }


            this.$confirm('你确定该物品是你丢失的物品吗？', '申请认领', {
