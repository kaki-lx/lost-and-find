      ...
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item label="关键词" style="margin-bottom: 10px">
          <el-input v-model.trim="searchForm.keyword" placeholder="请输入关键词" @change="keywordInput()"
            @clear="keyworkClear()" clearable></el-input>
        </el-form-item>
      ...
      <el-table-column sortable fixed="right" prop="isFound" label="状态" width="80">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isFound == 1 ? 'primary' : (scope.row.isFound == 0 ? 'success' : 'warning')"
            disable-transitions>
            {{ scope.row.isFound == 1 ? '待认领' : (scope.row.isFound == 0 ? '已认领' : '处理中') }}
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="60%" top="10px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row :gutter="20" v-if="dialogDisabled">
          <el-col :span="8">
            <el-form-item label="认领人电话号码" prop="handlerPhone" :label-width="80">
          ...
          <el-col :span="8">
            <el-form-item label="认领人微信号" prop="handlerWechat" :label-width="80">
              <el-tag type="success" style="font-size: 16px; width: 100%;">{{ form.handlerWechat == '' ?
                '未填写' : form.handlerWechat }}</el-tag>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="认领人QQ" prop="handlerQQ" :label-width="80">
  ...
  methods: {
    ...
    typeSelectClear() {
      if (this.searchForm.keyword) {
        ...
        this.userClaimList = this.$store.getters.getUserClaim
        // 2.对关键词进行筛选
        const claimList_resume = this.userClaimList.filter(item => item.resume.includes(this.searchForm.keyword))
        const claimList_description = this.userClaimList.filter(item => item.description.includes(this.searchForm.keyword))
        const claimList_location = this.userClaimList.filter(item => item.location.includes(this.searchForm.keyword))
        const array = [...claimList_resume, ...claimList_description, ...claimList_location] // 存在重复数据
        this.userClaimList = array.filter(function (item, index, arr) { return arr.indexOf(item, 0) === index })  // 数据去重
      } else {
        // 1.初始化
        this.userClaimList = this.$store.getters.getUserClaim
      }
    ...
    typeSelectChange() {
      if (this.searchForm.keyword) {
        ...
        this.userClaimList = array.filter(function (item, index, arr) { return arr.indexOf(item, 0) === index })  // 数据去重
        // 3.对类型进行筛选
        this.userClaimList = this.userClaimList.filter(item => item.type == this.searchForm.type)
      } else {
        // 1.初始化
        this.userClaimList = this.$store.getters.getUserClaim
        // 2.对类型进行筛选
