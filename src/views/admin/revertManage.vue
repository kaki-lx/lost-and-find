<template>
    ...
    <div class="container">
        <div class="ONE">
            <div class="left">
                <p><span class="link" @click="toIndex">首页</span> > 数据管理 > 寻物信息管理</p>
            </div>
            <div class="right">
        ...
        <div class="TWO">
            <el-table :data="viewList" border highlight-current-row style="width: 100%; text-align: center;">
                ...
                <el-table-column prop="handlerNickname" label="归还者" sortable width="100" header-align="center"
                    align="center">
                </el-table-column>
                ...
                <el-table-column prop="isFound" label="状态" sortable width="80" header-align="center" align="center">
                    <template slot-scope="scope">
                        <el-tag class="isFoundTag"
                            ...
                            {{ scope.row.isFound == 1 ? '待归还' : (scope.row.isFound == 0 ? '已归还' : (scope.row.isFound
                            == 2 ? '处理中' : '申请中'))}}
                        </el-tag>
                    </template>
                </el-table-column>
        ...
        <div class="THREE">
            <el-pagination @current-change="handleCurrentChange" :current-page="currentPage" :page-size="pageSize"
                layout="total, prev, pager, next, jumper" :total="revertList.length">
            </el-pagination>
        </div>

        <el-dialog class="view-dialog" title="寻物启事信息" :visible.sync="dialogVisible" width="64%" top="10px">
            <div class="dialog-container">
                <div class="top">
                    <el-steps :space="200" align-center
                        ...
                        <el-step title="申请归还" :description="view.revertDate"> </el-step>
                        <el-step title="申请归还通过" :description="view.verifyDate"> </el-step>
                        <el-step title="确认归还" :description="view.finishDate"></el-step>
                    </el-steps>
                </div>
                <div class="bottom">
                    <div class="left">
                        ...
                        <el-image v-if="!view.article_pic" style="height: 280px;width: 200px;"
                            :src="require('../../../public/no_pic/noArticle.png')"></el-image>
                    </div>
                    <div class="right">
                        <el-descriptions title="" border :column="2"
                            ...
                            <el-descriptions-item label="寻物ID">{{ view.id }}</el-descriptions-item>
                        ...
                        <el-descriptions title="" border :column="2"
                            :labelStyle="{ 'width': '20%', 'text-align': 'center' }"
                            :contentStyle="{ 'width': '30%', 'text-align': 'center' }">
                            <el-descriptions-item label="失主ID">{{ view.initiatorId }}</el-descriptions-item>
                            <el-descriptions-item label="归还者ID">{{ view.handlerId }}</el-descriptions-item>

                            <el-descriptions-item label="失主昵称">{{ view.initiatorNickname }}</el-descriptions-item>
                            <el-descriptions-item label="归还者昵称">{{ view.handlerNickname }}</el-descriptions-item>

                            <el-descriptions-item label="失主联系方式">{{ view.initiatorPhone }}</el-descriptions-item>
                            <el-descriptions-item label="归还者联系方式">{{ view.handlerPhone }}</el-descriptions-item>

                            <el-descriptions-item label="失主微信">{{ view.initiatorWechat }}</el-descriptions-item>
                            <el-descriptions-item label="归还者微信">{{ view.handlerWechat }}</el-descriptions-item>

                            <el-descriptions-item label="失主QQ">{{ view.initiatorQQ }}</el-descriptions-item>
                            <el-descriptions-item label="归还者QQ">{{ view.handlerQQ }}</el-descriptions-item>
                        </el-descriptions>
        ...
        <el-dialog class="edit-dialog" title="招领信息" :visible.sync="editDialogVisible" width="90%" top="80px">
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="10" style="border-left: solid 1px black;">
                        ...
                        <el-row :gutter="20">
                            ...
                            <el-col :span="12">
                                ...
                                <el-form-item label="归还时间" prop="revertDate"
                                    :label-width="formLabelWidth">{{ form.claimDate }}
                                </el-form-item>
                            </el-col>
                            <!-- 第二列 -->
                            <el-col :span="12">
                                <el-form-item label="发布时间" prop="createDate"
                                    :label-width="formLabelWidth">{{ form.createDate }}
                                </el-form-item>
                                <el-form-item label="完成时间" prop="finishDate"
                                    :label-width="formLabelWidth">{{ form.finishDate }}
                                </el-form-item>
                            </el-col>
                        </el-row>
                    </el-col>
                    ...
                    <el-col :span="10" style="border-left: solid 1px black;">
                        ...
                        <el-row :gutter="20">
                            <!-- 第一列 -->
                            <el-col :span="12">
                                <el-form-item label="拾物者ID" prop="initiatorId"
                                    :label-width="formLabelWidth">{{ form.initiatorId }} </el-form-item>
                                <el-form-item label="昵称" prop="initiatorNickname"
                                    :label-width="formLabelWidth">{{ form.initiatorNickname }} </el-form-item>
                            ...
                            <!-- 第二列 -->
                            <el-col :span="12">
                                <el-form-item label="认领者ID" prop="handlerId"
                                    :label-width="formLabelWidth">{{ form.handlerId }}
                                </el-form-item>
                                <el-form-item label="昵称" prop="handlerNickname"
                                    :label-width="formLabelWidth">{{ form.handlerNickname }} </el-form-item>
        ...
        <el-dialog class="articlePic-dialog" title="修改物品图片" :visible.sync="articlePicDialogVisible" width="30%">
            <span>上传物品图片
                <el-upload action="http://127.0.0.1:3007/my/uploadRevertPic" ref="upload" :headers="myHeaders"
        ...
        <el-dialog class="status-dialog" title="状态修改" :visible.sync="statusDialogVisible" width="30%" top="10px">
            <div class="dialog-container">
                <el-button type="primary" :disabled="status.isFound == 1" @click="editStatusRevert">待归还</el-button>
                ...
                <el-button type="success" :disabled="status.isFound == 0 || status.isFound == 1"
                    @click="editStatusReverted">已归还</el-button>
            </div>
        </el-dialog>
...
export default {
    ...
    methods: {
        getRevertList() {
            http({
                ...
                if (res.data.status === 0) {
                    ...
                    this.viewList = [...this.revertList.slice(this.currentPage * this.pageSize - this.pageSize, this.currentPage * this.pageSize)]
                }
            }).catch((err) => {
        ...
        editStatus(row) {
            ...
            this.status = { ...row }
            if (row.isFound === 3) {
                this.$message.warning('请到【申请处理】->【归还申请】中处理！')
            } else {
                this.statusDialogVisible = true
            }
        },
        // 修改状态 处理中/已归还 -> 待归还 的按钮
        editStatusRevert() {
            // true: 已归还 -> 待归还，  false: 处理中 -> 待归还
        ...
        // 修改状态 待归还/已归还 -> 处理中 的按钮
        editStatusProcessing() {
            // 如果信息中有处理人信息 说明是 已归还 -> 处理中, 反之 待归还 -> 处理中
